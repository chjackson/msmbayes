...
Essentially $h()$ is determined by minimising the Kullback-Leibler discrepancy between the phase-type distribution and the Weibull, for a series of points on a fine grid.  The optimal solutions are interpolated using a Hermite cubic spline, and extended beyond the grid boundaries with a constant, to define a full time-to-event distribution function for positive times $x$.  This is done for a scale parameter of $b=1$, then generalised to other values of $b$ through the accelerated failure time property of the Weibull or Gamma and phase-type models.  This process will be described more formally in a forthcoming paper, but for now, the code is in `data-raw/kl_pointwise.R` in the package source.
