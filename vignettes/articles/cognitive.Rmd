...
Additional files sourced from this one (the data and additional R code) are available from `vignettes/articles` in the `msmbayes` GitHub repository.
...
Since there is very little information in this dataset about death,  we obtain strong priors on the death rates, and effect of age and sex on these, from national mortality statistics.   Other effects are given weakly informative priors: a log-normal(0, 1) distribution supports hazard ratios up to around `exp(2) = 7`, which we suppose would be surprising in the context of demographic effects on the risk of cognitive decline.   The priors are stored in the file `elsa_priors.R` (available in `vignettes/articles` in the `msmbayes` GitHub repository), and code used to derive them is in Supplementary Appendix 4 of the paper. 
