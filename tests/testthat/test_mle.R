...
test_that("msmbayes with flat priors: mode agrees with MLE from msm",{
  ...
  ll <- loglik(fitb)
  expect_equal(-2*ll$mode[ll$name=="loglik"], fitm$minus2loglik)
...
test_that("msmbayes with flat priors agrees with msm: models with covariates",{
  ...
  ll <- loglik(fitbc)
  expect_equal(-2*ll$mode[ll$name=="loglik"], fitmc$minus2loglik)
...
test_that("msmbayes with flat priors and misclassification agrees with msm",{
  ...
  ll <- loglik(drawse)
  expect_equal(-2*ll$mode[ll$name=="loglik"], lik_msm$minus2loglik)
