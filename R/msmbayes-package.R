...
#' @importFrom stats delete.response na.omit reshape setNames terms quantile runif qnorm pexp plogis qlogis rexp approx rnorm as.formula 
#' @importFrom posterior as_draws as_draws_matrix rhat ess_bulk rvar ndraws rvar_sum "%**%" rdo rvar_sum draws_of merge_chains is_rvar rfun
...
#' @importFrom magrittr "%>%"
#' @importFrom dplyr mutate select filter slice relocate left_join n arrange group_by ungroup pull row_number full_join inner_join join_by if_else summarise matches all_of rename across all_of everything 
