...
data {
  ...
  // For phasetype approx with competing exit states.  RENAME PABS PADEST? 
...
transformed parameters {
  ...
  vector[npadest] padest;    // transition probabilities from phaseapprox states to competing destinations
  {
	...
	if (dest_base[i]==1) {
	  odds[i] = 1;
	  sumodds[dest_state[i]] = odds[i];
	}  else {
	  odds[i] = exp(logoddsabs[loind[i]]);
	  sumodds[dest_state[i]] = sumodds[dest_state[i]] + odds[i];
	}
      }
      for (i in 1:npadest){
	padest[i] = odds[i] / sumodds[dest_state[i]];
      }
    }
